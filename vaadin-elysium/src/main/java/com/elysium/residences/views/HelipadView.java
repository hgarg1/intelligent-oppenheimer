...
@Route("helipad")
public class HelipadView extends Div {

    private static final Logger logger = LoggerFactory.getLogger(HelipadView.class);

    public HelipadView() {
        addClassName("helipad-viewport");
        loadTemplate();
    }

    private void loadTemplate() {
        try (InputStream inputStream = getClass().getResourceAsStream("/META-INF/resources/helipad/index.html")) {
            if (inputStream == null) {
                logger.error("Could not find helipad index.html in resources!");
                getElement().setProperty("innerHTML", "<div style='color:red; padding:50px;'>Error: Helipad page template not found.</div>");
                return;
            }
            ...
            if (bodyStart != -1 && bodyEnd != -1) {
                ...
                logger.info("Successfully injected luxury helipad landing DOM.");
            } else {
                getElement().setProperty("innerHTML", htmlContent);
                logger.warn("Injected raw helipad index.html (missing body tags).");
            }
        } catch (Exception e) {
            logger.error("Failed to load helipad page template", e);
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        attachEvent.getUI().getPage().executeJs(
            ...
            "  });" +
            "}" +
            "const container = document.querySelector('.helipad-viewport');" +
            "if (container) {" +
