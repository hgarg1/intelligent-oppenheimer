...
@Route("privacy")
public class PrivacyView extends Div {

    private static final Logger logger = LoggerFactory.getLogger(PrivacyView.class);

    public PrivacyView() {
        addClassName("privacy-viewport");
        loadTemplate();
    }

    private void loadTemplate() {
        try (InputStream inputStream = getClass().getResourceAsStream("/META-INF/resources/privacy/index.html")) {
            if (inputStream == null) {
                logger.error("Could not find privacy index.html in resources!");
                getElement().setProperty("innerHTML", "<div style='color:red; padding:50px;'>Error: Privacy page template not found.</div>");
                return;
            }
            ...
            if (bodyStart != -1 && bodyEnd != -1) {
                ...
                logger.info("Successfully injected luxury privacy landing DOM.");
            } else {
                getElement().setProperty("innerHTML", htmlContent);
                logger.warn("Injected raw privacy index.html (missing body tags).");
            }
        } catch (Exception e) {
            logger.error("Failed to load privacy page template", e);
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        attachEvent.getUI().getPage().executeJs(
            ...
            "  });" +
            "}" +
            "const container = document.querySelector('.privacy-viewport');" +
            "if (container) {" +
